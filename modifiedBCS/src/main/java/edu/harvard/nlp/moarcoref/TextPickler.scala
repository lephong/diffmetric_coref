...
object TextPickler {
 ...
 def writeFullMentandCtx(docGraphs: Seq[DocumentGraph],fiName: String, lowercase:Boolean = false){
    ...
    for (dg <- docGraphs){
      ...
      while (i < dg.size){
        ...
        if (lowercase){
            ...
            for (u <- 5 to 1 by -1)
              pw.print(ment.contextWordOrPlaceholder(-u) + " ");
            pw.print(" ||| ");
            pw.print(ment.spanToString + " ||| ");
            for (u <- 1 to 5)
              pw.print(ment.contextWordOrPlaceholder(ment.words.size + u - 1) + " ");
            pw.print(" ||| ")
            pw.print(ment.headString + " ||| ")
            pw.print(ment.governor + " ")
        }
        i += 1;
      }
      pw.println();
  ...
  def writeFullText(docGraphs: Seq[DocumentGraph],fiName: String, lowercase:Boolean = false){
    val pw = new PrintWriter(fiName);
    for (dg <- docGraphs){
      for (sentence <- dg.corefDoc.rawDoc.words) {
        pw.print(sentence.mkString(" "));
        pw.println();
      }
      ...
      while (i < dg.size){
        val ment = dg.corefDoc.predMentions(i);
        pw.print(ment.sentIdx.toString + " " + ment.startIdx.toString + " " + ment.endIdx.toString + " " +
          ment.headIdx.toString + " " + ment.mentionType.toString + " " + ment.nerString.toString + " " +
          ment.number.toString + " " + ment.gender.toString);
        pw.println();
        i += 1;
      }
      pw.println();
