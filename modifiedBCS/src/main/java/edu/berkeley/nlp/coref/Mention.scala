...
//  val cachedCanonicalPronConjStr = if (mentionType == MentionType.PRONOMINAL) {
  ...
  private def governorHelper(pos: Boolean, dir: Boolean=false) = {
    val parentIdx = rawDoc.trees(sentIdx).childParentDepMap(headIdx);
    if (parentIdx == -1) {
      "[ROOT]"
    } else {
      (if (dir) (if (headIdx < parentIdx) "L" else "R") + "-" else "") + (if (pos) rawDoc.pos(sentIdx)(parentIdx) else rawDoc.words(sentIdx)(parentIdx));
    }
  }
