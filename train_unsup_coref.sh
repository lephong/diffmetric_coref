...
#DEV_NA_PREFIX=$DATA_DIR/train/na.5.txt-na-
#DEV_NA_LEX=$DATA_DIR/train/na_lex.5.txt
#DEV_PW_PREFIX=$DATA_DIR/train/pw.5.txt-pw-
#DEV_ORACLE_CLUSTER=$DATA_DIR/train/OPCs.5.txt

MENT_MAP_FILE=$DATA_DIR/anaphReMapping.txt
PW_MAP_FILE=$DATA_DIR/pwReMapping.txt


EXPERIMENT_DIR=$PROJECT_HOME/experiment_sup/
INIT_GEN_MODEL=$EXPERIMENT_DIR/init_gen.model
INIT_DISCR_MODEL=$EXPERIMENT_DIR/init_discr.model
INIT_LM_MODEL=$PROJECT_HOME/src/lm/train/

VOCA=$PROJECT_HOME/src/lm/small_voca.txt
...
for ((epoch=2;epoch<$N_EPOCHS;epoch++))
do
    echo "=========================== epoch " $epoch "============================="
    ...
    do
        ...
        if [ $epoch -eq 0 ] && [ $portion -eq 0 ] ; then
            python3 -u src/unsup_coref.py $GPU \
                ...
                #--init_language_model_path $INIT_LM_MODEL
                #--init_gen_model_path $INIT_GEN_MODEL \
                #--init_discr_model_path $INIT_DISCR_MODEL
